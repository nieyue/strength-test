...
private Color[] bordercolor={Color.black,Color.blue,Color.cyan,Color.darkGray,Color.gray,Color.green,Color.lightGray,Color.magenta,Color.orange,Color.pink,Color.red,Color.white,Color.yellow};
	...
	//将图片以字节形式写到InputStream里
	public ByteArrayOutputStream createInuptStream(String adminName) throws Exception {
		//获取随机字符串
		String str = this.creatRandomString();
		BufferedImage image = this.createImage(str);
		BoundHashOperations<String, Object, Object> hash = stringRedisTemplate.boundHashOps("verificationCode");
		hash.put("adminName",str);
		ByteArrayOutputStream output = new ByteArrayOutputStream();
		ImageOutputStream imageOut = ImageIO.createImageOutputStream(output);
		ImageIO.write(image, "JPEG", imageOut);
		imageOut.close();
	...
	public ByteArrayOutputStream execute(String adminName) throws Exception {
		return createInuptStream(adminName);
	}
